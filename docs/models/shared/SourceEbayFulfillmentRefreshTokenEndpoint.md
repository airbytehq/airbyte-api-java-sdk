# SourceEbayFulfillmentRefreshTokenEndpoint

## Example Usage

```java
import com.airbyte.api.models.shared.SourceEbayFulfillmentRefreshTokenEndpoint;

SourceEbayFulfillmentRefreshTokenEndpoint value = SourceEbayFulfillmentRefreshTokenEndpoint.HTTPS_API_EBAY_COM_IDENTITY_V1_OAUTH2_TOKEN;
```


## Values

| Name                                                  | Value                                                 |
| ----------------------------------------------------- | ----------------------------------------------------- |
| `HTTPS_API_EBAY_COM_IDENTITY_V1_OAUTH2_TOKEN`         | https://api.ebay.com/identity/v1/oauth2/token         |
| `HTTPS_API_SANDBOX_EBAY_COM_IDENTITY_V1_OAUTH2_TOKEN` | https://api.sandbox.ebay.com/identity/v1/oauth2/token |